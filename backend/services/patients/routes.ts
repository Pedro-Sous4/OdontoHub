...
patientsRouter.get('/', async (req: AuthRequest, res) => {
  ...
  const result = await query(
    'SELECT id, tenant_id, nome, cpf, telefone, email, data_nascimento, created_at FROM patients WHERE tenant_id = $1 ORDER BY created_at DESC LIMIT 100',
    [tenantId]
  );
...
patientsRouter.post('/', requireRole(['admin', 'receptionist', 'dentist']), async (req: AuthRequest, res) => {
  ...
  const created = await query<{ id: string }>(
    `INSERT INTO patients (tenant_id, nome, cpf, telefone, email, data_nascimento)
     VALUES ($1, $2, $3, $4, $5, $6)
     RETURNING id`,
    [tenantId, nome, cpf, telefone, email, dataNascimento]
  );
